...
Page({
  ...
  buyNow() {
    ...
    wx.showModal({
      title: "确认购买",
      content: `您确定要购买「${detail.title}」吗？价格：¥${detail.price}`,
      success: (res) => {
        if (res.confirm) {
          wx.showLoading({ title: "处理中..." })
          wx.request({
            url: "http://127.0.0.1:8080/campus/purchaseRecord/create",
            method: "POST",
            data: {
              secondId: detail.id,
            ...
            success: (resp) => {
              ...
              if (resp.data.code === 200) {
                ...
                setTimeout(() => {
                  wx.navigateTo({
                    url: "/pages/purchase/pay?recordId=" + resp.data.data
                  })
                }, 1000)
              } else {
                wx.showToast({ title: resp.data.msg || "创建订单失败", icon: "none" })
              }
            },
            fail: () => {
              wx.hideLoading()
